...
import { updatePublicationAction } from '../../../app/admin/publicaciones/actions';
import { AdminPublication } from '../../../lib/admin/admin-publications';
...
export default function EditPublicationForm({
  ...
  const [publicationType, setPublicationType] = useState<'book' | 'album' | 'special_work'>(initialData.publication_type || 'book');
  const [authorText, setAuthorText] = useState(initialData.author_text || '');
  const [publicationYear, setPublicationYear] = useState<number | ''>(initialData.publication_year === null ? '' : initialData.publication_year);
  const [publisherInstitutionId, setPublisherInstitutionId] = useState(initialData.publisher_institution_id || '');
  const [coverImageAssetId, setCoverImageAssetId] = useState(initialData.cover_image_asset_id || '');
  const [sourceReference, setSourceReference] = useState(initialData.source_reference || '');
  const [status, setStatus] = useState<'draft' | 'review' | 'published' | 'archived' | 'rejected'>(initialData.status || 'draft');
  const [visibility, setVisibility] = useState<'public' | 'subscribers' | 'institutional' | 'private'>(initialData.visibility || 'public');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (status === 'published' && initialData.status !== 'published') {
    ...
    try {
      const res = await updatePublicationAction(publicationId, updatedPublicationData);
      if (res.success) {
        window.location.href = '/admin/publicaciones?guardado=1';
      } else {
        setErrorMsg(res.error || 'Ocurrió un error al intentar guardar la publicación.');
        setLoading(false);
      }
    } catch (err: any) {
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="bg-amber-50 border-l-4 border-amber-500 p-4 rounded-r-md">
        <p className="text-xs text-amber-800 font-bold uppercase tracking-wider font-mono">
          Edición de la publicación: {initialData.title}. El slug único no se modificará para conservar los enlaces de búsqueda y navegación existentes.
        </p>
      </div>
