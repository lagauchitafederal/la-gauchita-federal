...
import { createEphemerisAction } from '../../../app/admin/efemerides/actions';

interface NewEphemerisFormProps {
...
export default function NewEphemerisForm({
  categories,
  regions,
  provinces,
  municipalities
}: NewEphemerisFormProps) {
  // Form states
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await createEphemerisAction(newEphemerisData);
      if (res.success) {
        window.location.href = '/admin/efemerides?creado=1';
      } else {
        setErrorMsg(res.error || 'Ocurrió un error al intentar crear la efeméride.');
        setLoading(false);
      }
    } catch (err: any) {
