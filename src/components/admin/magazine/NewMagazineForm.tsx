...
import { createMagazineAction } from '../../../app/admin/revista/actions';

interface NewMagazineFormProps {
...
export default function NewMagazineForm({
  institutions,
  covers,
  pdfs,
  defaultPublisherId,
}: NewMagazineFormProps) {
  // Form states
  ...
  const [toc, setToc] = useState<IndexEntry[]>([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await createMagazineAction(newMagazineData);
      if (res.success) {
        window.location.href = '/admin/revista?creado=1';
      } else {
        setErrorMsg(res.error || 'Ocurrió un error al intentar crear la edición.');
        setLoading(false);
      }
    } catch (err: any) {
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="bg-amber-50 border-l-4 border-amber-500 p-4 rounded-r-md">
        <p className="text-xs text-amber-800 font-bold uppercase tracking-wider font-mono">
          El slug se generará automáticamente a partir del nombre del editor y número. Los archivos asociados deben ser existentes en la biblioteca.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-stone-beige rounded-lg shadow-sm p-6 sm:p-8 flex flex-col gap-6">
        ...
        <div className="border-t border-stone-200 pt-6 flex justify-end gap-3 font-mono">
          ...
          >
            {loading ? 'Guardando...' : 'Crear Edición'}
          </button>
        </div>
